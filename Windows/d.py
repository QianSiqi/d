...
try:
    ...
    d = input('Windows安装easygui和pygame和pgzero和flask库了吗？[安装了/还没有]')
    if d == '还没有':
        o.system('pip install easygui')
        o.system('sudo apt-get install python3-tk')
        o.system('pip install pygame')
        o.system(
            'pip install numpy -i http://pypi.douban.com/simple --trusted-host pypi.douban.com')
        o.system(
            'pip install pgzero -i http://pypi.douban.com/simple --trusted-host pypi.douban.com')
        o.system(
            'pip install flask -i http://pypi.douban.com/simple --trusted-host pypi.douban.com')
    if d == '安装了':
        x = open('sett.ngm', 'w')
        x.write(d + '第三方库\n')
    a = input('请输入你的Windows的系统和版本（示例：Windows11-22h2）：')
    x.write(a + '\n')
...
def flask模板():
    try:
        ...
        fkmb.read()
        o.system('python flaskk.py')
    except FileNotFoundError:
        fkmbb = open('flaskk.py', 'w',encoding='utf-8')
        fkmbb.write('''
from flask import Flask
app = Flask('__name__')
@app.route('/')
...
<html>
    ...
    <button onclick="window.location.href = '/ngmdw/'">别点我！！！！！！</button>
</html>
    ''')
